...
import com.wxf.data.graphicSearch.MatrixGraphic;
...
public class MatrixUDG {
	...
	private EData[] getEdges() {
		...
		for (int i = 0; i < mVexs.length; i++) {
			for (int j = i + 1; j < mVexs.length; j++) {
				if (mMatrix[i][j] != INF) {
					edges[index++] = new EData(mVexs[i], mVexs[j],
							mMatrix[i][j]);
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		// pG = new MatrixUDG(vexs1, matrix);

		// **********************测试*********************
		int n = 7;
		MatrixGraphic graphic = new MatrixGraphic(7, INF);
		// graphic.addVertex(new Object[] { "A", "B", "C", "D", "E", "F", "G"
		// });
		graphic.addVertex(vexs);
